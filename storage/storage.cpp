...
{
  LOG(LINFO, ("Registering country files"));
  CHECK(localFile, ());
  localFile->SyncWithDisk();
  ...
  // World.mwm and WorldCoast.mwm might be bundled
  if (!localFile->IsInBundle())
  {
    // CalculateHealpixForRegion(countryId, localFile);
  }
...
CountryTree::Node const & Storage::GetRootNode() const { return m_countries.GetRoot(); }
